...
#### 2.3 Update Firebase Config in Client
...
### Step 4: Set Up Admin User (First-Time Setup)
...
This creates the first Owner (Super Admin) account with custom claims.
...
Create `client/.env` for local emulator testing:
...
# Or build from client directory
cd client
npm run build
```
