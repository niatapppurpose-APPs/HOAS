...
/**
 * AnimatedLogoutButton - A fun animated logout button with a walking figure animation
 * @param {Object} props
 * @param {Function} props.onLogout - Callback function when logout animation completes
 * @param {string} props.variant - 'light' or 'dark' theme variant (default: 'dark')
 * @param {string} props.className - Additional CSS classes
 * @param {string} props.text - Button text (default: 'Log Out')
 */
const AnimatedLogoutButton = ({ 
  ...
  const [cssVars, setCssVars] = useState(logoutButtonStates.default);
  const [animationClasses, setAnimationClasses] = useState([]);
  const buttonRef = useRef(null);

  const updateButtonState = useCallback((state) => {
    if (logoutButtonStates[state]) {
      setButtonState(state);
      setCssVars(prev => ({ ...prev, ...logoutButtonStates[state] }));
    }
  }, []);
  ...
  const handleClick = () => {
    if (buttonState === 'default' || buttonState === 'hover') {
      setAnimationClasses(['clicked']);
      updateButtonState('walking1');

      setTimeout(() => {
        setAnimationClasses(['clicked', 'door-slammed']);
        updateButtonState('walking2');

        setTimeout(() => {
          setAnimationClasses(['clicked', 'door-slammed', 'falling']);
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                setAnimationClasses([]);
                updateButtonState('default');
                // Trigger the actual logout callback
                if (onLogout) {
                  onLogout();
                }
              }, 1000);
  ...
  const buttonClasses = [
    ...
    variant === 'light' ? 'animated-logout-button--light' : 'animated-logout-button--dark',
    ...animationClasses,
    className
  ].filter(Boolean).join(' ');
