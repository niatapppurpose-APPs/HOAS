...
const EmergencyLocationMonitor = ({
  title = "Emergency Live Locations",
  tone = "warden",
}) => {
  ...
  const selectedLocation = useMemo(
    () =>
      locations.find((location) => location.studentId === selectedStudentId) ||
      locations[0] ||
      null,
    ...
    try {
      ...
      const data = raw.map((item) => {
        ...
        return {
          ...
          lastUpdatedAt:
            item.lastUpdatedAt ?? item.lastUpdateAt ?? item.updatedAt,
        };
      });
      setLocations(data);
      // The monitor response is authoritative for staff views. If there are
      // no active sessions, clear any alert persisted from a missed socket event.
      if (data.length === 0 && emergencyActive) {
        clearAlert();
      }
      ...
      setTrackByStudent((prev) => {
        ...
        Object.keys(next).forEach((studentId) => {
          if (!activeIds.has(studentId)) delete next[studentId];
        });
        return next;
      });
  ...
  return (
    <>
      <section
        className={`relative overflow-hidden rounded-[2rem] border p-6 md:p-8 backdrop-blur-2xl transition-all duration-500 shadow-2xl ${isDark ? "bg-slate-900/40 border-slate-700/50 shadow-black/50" : "bg-white/60 border-white/60 shadow-indigo-900/5"} tone-${tone}`}
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-5 mb-8">
          <div className="space-y-1.5">
            <div
              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase mb-2 border 
            ...
            <h3
              className={`text-3xl md:text-4xl font-extrabold tracking-tight ${isDark ? "text-white" : "text-slate-900"} bg-clip-text text-transparent bg-gradient-to-r from-red-500 to-rose-400`}
            >
              {title}
            </h3>
            <p
              className={`text-sm font-medium ${isDark ? "text-slate-400" : "text-slate-500"} flex items-center gap-2`}
            >
              Monitor exact positioning of students requesting immediate
              assistance.
            </p>
          </div>
          <div className="flex gap-4">
            ...
            {emergencyActive && (
              <button
                ...
                className={`inline-flex items-center gap-2 px-4 py-3 rounded-xl font-bold text-sm border transition-colors ${
                  isMuted
                    ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-600 hover:bg-emerald-500/20"
                    : "bg-red-500/10 border-red-500/30 text-red-600 hover:bg-red-500/20"
                }`}
                title={
                  isMuted ? "Unmute emergency siren" : "Mute emergency siren"
                }
                aria-label={
                  isMuted ? "Unmute emergency siren" : "Mute emergency siren"
                }
        ...
        <div
          className={`relative z-10 mb-6 flex items-center gap-4 p-4 rounded-2xl border backdrop-blur-md transition-colors 
        ...
        >
          <div
            className={`p-3 rounded-xl ${locations.length > 0 ? "bg-red-500 text-white" : "bg-emerald-500 text-white"} shadow-lg`}
          >
            {locations.length > 0 ? (
          ...
          <div>
            ...
            <p
              className={`text-xs font-semibold ${locations.length > 0 ? "text-red-500" : "text-emerald-500"}`}
            >
              {locations.length > 0
                ? "Immediate action may be required."
                : "All clear. No active distress signals."}
            </p>
          </div>
        </div>
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start h-full">
          ...
          <div className="lg:col-span-4 max-h-[500px] overflow-y-auto pr-2 space-y-3 custom-scrollbar">
            {loading && locations.length === 0 ? (
              <div
                className={`p-8 rounded-2xl text-center border border-dashed ${isDark ? "border-slate-700 bg-slate-800/30" : "border-slate-300 bg-slate-50"}`}
              >
              ...
              <div
                className={`p-10 rounded-2xl text-center border border-dashed flex flex-col items-center justify-center
              ...
              >
                ...
                >
                  No active tracking sessions at this moment.
                </p>
              </div>
            ) : (
              locations.map((item) => {
                ...
                const isGpsLost =
                  Date.now() -
                    new Date(item.lastUpdatedAt || item.sharedAt).getTime() >
                  25000;
                return (
                  <div key={item.studentId} className="space-y-2">
                    <button
                      type="button"
                      onClick={() => {
                        setSelectedStudentId(item.studentId);
                        setExpandedStudentId((current) =>
                          current === item.studentId ? null : item.studentId,
                        );
                      }}
                      className={`w-full text-left p-4 rounded-2xl transition-all duration-300 border-2 relative overflow-hidden group hover:-translate-y-1
                        ${
                          isActive
                            ? isDark
                              ? "bg-slate-800 border-red-500/50 shadow-[0_8px_20px_rgba(239,68,68,0.15)]"
                              : "bg-white border-red-500 shadow-xl shadow-red-500/10"
                            : isDark
                              ? "bg-slate-800/40 border-slate-700/50 hover:bg-slate-800 hover:border-slate-600"
                              : "bg-white/80 border-transparent hover:bg-white hover:shadow-lg"
                        }`}
                    >
                      {isActive && (
                      ...
                      <div className="flex items-start justify-between gap-3 relative z-10">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 justify-between">
                            <h5
                              className={`font-bold text-base flex items-center gap-2 truncate ${isDark ? "text-white" : "text-slate-900"}`}
                            >
                            ...
                            {isGpsLost && (
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-amber-500/20 text-amber-500 border border-amber-500/30 shrink-0">
                                <AlertTriangle size={10} /> GPS Lost
                              </span>
                            )}
                          </div>
                          <p
                          ...
                          <p
                            className={`flex items-center gap-1.5 text-[10px] uppercase tracking-widest font-bold mt-3 ${isGpsLost ? "text-amber-500" : isDark ? "text-slate-500" : "text-slate-400"}`}
                          >
                            <Activity size={10} />{" "}
                            {isGpsLost ? "Last recorded: " : "Pinged "}{" "}
                            {formatDateTime(
                              item.lastUpdatedAt || item.sharedAt,
                            )}
                          </p>
                        </div>
                        <ChevronRight
                          className={`w-5 h-5 transition-transform ${isActive ? "text-red-500 translate-x-1" : isDark ? "text-slate-600 group-hover:text-slate-400" : "text-slate-300 group-hover:text-slate-500"}`}
                        />
                      </div>
                    </button>
                    {isActive && expandedStudentId === item.studentId && (
                      <div
                        className={`rounded-2xl border p-3 text-xs ${isDark ? "bg-slate-900/80 border-slate-700 text-slate-300" : "bg-red-50/70 border-red-100 text-slate-600"}`}
                      >
                        <p
                          className={`mb-2 font-bold uppercase tracking-widest ${isDark ? "text-red-300" : "text-red-600"}`}
                        >
                          Emergency Details
                        </p>
                        <div className="grid grid-cols-2 gap-x-3 gap-y-2">
                          ...
                          <span
                            ...
                            title={item.sessionId || item._id}
                          >
                            {item.sessionId || item._id || "N/A"}
                          </span>
                          ...
                          <span
                            className={`text-right font-semibold ${isGpsLost ? "text-amber-500" : "text-emerald-500"}`}
                          >
                            {isGpsLost ? "Signal lost" : "Live"}
                          </span>
          ...
          {/* Map View */}
          <div
            className={`lg:col-span-8 min-h-[400px] h-[500px] relative rounded-[2rem] overflow-hidden border shadow-2xl z-10 ${isDark ? "dark-map-wrapper bg-slate-900 border-slate-700/60" : "bg-slate-100 border-slate-300 light-map-wrapper"}`}
          >
            {selectedLocation ? (
              (() => {
                const isSelectedGpsLost =
                  Date.now() -
                    new Date(
                      selectedLocation.lastUpdatedAt ||
                        selectedLocation.sharedAt,
                    ).getTime() >
                  25000;
                return (
                  <>
                    <div
                      className={`absolute top-4 left-4 z-[1000] px-4 py-2 rounded-xl backdrop-blur-xl border shadow-lg flex items-center gap-2
                    ${isSelectedGpsLost ? "bg-amber-950/90 border-amber-500/50 text-amber-200" : isDark ? "bg-slate-900/80 border-slate-700 text-white" : "bg-white/90 border-white text-slate-800"}`}
                    >
                      {isSelectedGpsLost ? (
                      ...
                      <span className="text-xs font-bold uppercase tracking-widest">
                        ...
                        {isSelectedGpsLost ? "(GPS Signal Lost)" : ""}
                      </span>
                    </div>
                    ...
                    >
                      ...
                      <Circle
                        ...
                        radius={Math.max(
                          Number(selectedLocation.accuracy) || 25,
                          20,
                        )}
                        pathOptions={{
                          color: isSelectedGpsLost
                      ...
                      <CircleMarker
                        ...
                        pathOptions={{
                          color: "#fff",
                          fillColor: isSelectedGpsLost ? "#f59e0b" : "#ef4444",
                          fillOpacity: 1,
                          weight: 3,
                        }}
                      >
                        ...
                        >
                          {isSelectedGpsLost
                            ? `⚠️ ${selectedLocation.studentName} (LAST KNOWN POSITION)`
                            : selectedLocation.studentName}
                        </Tooltip>
                      </CircleMarker>
                    </MapContainer>
                    ...
                    {isSelectedGpsLost ? (
                      <div className="absolute bottom-4 left-4 z-[1000] px-3.5 py-2 rounded-xl backdrop-blur-md border border-amber-500/50 bg-amber-950/80 text-amber-300 flex items-center gap-2 text-xs font-bold shadow-lg">
                        <AlertTriangle className="w-4 h-4 text-amber-400 animate-pulse" />
                        <span>
                          GPS SIGNAL LOST — Displaying Last Recorded Position (
                          {formatDateTime(
                            selectedLocation.lastUpdatedAt ||
                              selectedLocation.sharedAt,
                          )}
                      ...
                      <div
                        className={`absolute bottom-4 left-4 z-[1000] px-3 py-1.5 rounded-lg backdrop-blur-md border flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest
              ...
              >
                ...
                <p className="text-sm font-semibold max-w-sm">
                  Map interface is currently dormant. Select an active emergency
                  session to initiate live student tracking.
                </p>
              </div>
            )}
          </div>
