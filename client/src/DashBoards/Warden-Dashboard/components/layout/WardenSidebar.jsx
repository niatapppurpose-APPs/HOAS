...
const WardenSidebar = ({ isCollapsed, setIsCollapsed, collegeLogo, managementData }) => {
  ...
  useEffect(() => {
    const storedValue = localStorage.getItem("wardenSidebarPinned");
    if (storedValue === "true") {
      setIsPinned(true);
  ...
  const handlePinClick = () => {
    ...
    localStorage.setItem('wardenSidebarPinned', String(newValue));
    if (newValue) {
      setIsCollapsed(false);
    } else {
      setIsCollapsed(true);
        ...
        >
            ...
            >
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  e.currentTarget.src = AppLogo4k;
                }}
                className={`relative w-full h-full rounded-xl object-cover border-2 border-slate-600/50 shadow-lg group-hover:border-orange-500/50 transition-all duration-300 group-hover:scale-105 ${effectiveLogo ? 'bg-white' : ''}`}
          ...
          <div className="flex items-center gap-1 flex-shrink-0 ml-1">
            ...
            <button
              ...
              className={`hidden lg:flex items-center justify-center w-8 h-8 rounded-lg transition-all duration-200 ${!showContent ? "opacity-0 pointer-events-none" : "opacity-100"} ${isPinned ? "bg-orange-600 text-white" : "hover:text-white"}`}
              style={{
          ...
          <div className="flex-1 px-3 space-y-1">
            ...
            {menuItems.map((item) => {
              ...
              const activeStyle = isActive
                ? { background: 'linear-gradient(90deg, #f97316, #f59e0b)', color: '#ffffff' }
                : {};
