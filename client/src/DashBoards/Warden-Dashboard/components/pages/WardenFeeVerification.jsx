    ...
    try {
      ...
      const list = (records || []).map(fee => {
        ...
        return {
          ...
          isVerifiedByWarden,
          isVerifiedByManagement: isVerifiedByMgmt,
        };
      });
      setRecords(list);
    } catch (error) {
        ...
        <div className="rounded-3xl p-6 md:p-8 border shadow-sm flex flex-col md:flex-row md:items-start justify-between gap-6"
          ...
          <div className="flex flex-col gap-4 w-full md:w-auto">
            <button
              ...
              disabled={loading}
              className="px-4 py-2 rounded-xl bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20 hover:bg-orange-500/20 transition-colors font-bold text-sm flex items-center gap-2 self-start"
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-3 sm:gap-4 shrink-0 overflow-visible z-10 w-full md:w-auto">
                      ...
                      <div className="flex items-center gap-2">
                        <div className={`px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg text-[10px] sm:text-xs font-bold flex items-center gap-1 sm:gap-1.5 shadow-sm ${
                          ...
                          MV: {record.isVerifiedByManagement ? 'Verified' : 'Pending'}
                        </div>
                        <div className={`px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg text-[10px] sm:text-xs font-bold flex items-center gap-1 sm:gap-1.5 shadow-sm ${
