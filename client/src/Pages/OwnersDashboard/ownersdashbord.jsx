...
const OwnersDashboard = () => {
        ...
        current.map((u) =>
          u.uid === updatedUser.uid
            ? {
                ...
                displayName: updatedUser.name || u.displayName,
  ...
  const handleStatusChange = async (userId, newStatus) => {
    ...
    try {
      ...
      if (error.message === "Operation timed out") {
        toast.error(
          `${newStatus} operation is taking longer than expected. Please check the result manually.`,
        );
      } else {
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      ...
      if (failCount === 0) {
        toast.success(
          `Successfully approved ${successCount} college${successCount > 1 ? "s" : ""}!`,
        );
      } else {
        toast.warning(
          `Approved ${successCount} colleges. ${failCount} failed.`,
        );
      }
    } catch (error) {
  ...
  const handleSelectAll = () => {
    ...
    if (
      selectedUsers.size === pendingOnPage.length &&
  ...
  const handleOpenDeleteModal = async (user) => {
    ...
    try {
      // Confirm deletion
      const confirmed = await toast.confirm(
        `Are you sure you want to delete management user "${user.displayName || user.email}"? This will remove their account Sub user accounts and College account`,
      );
      if (!confirmed) return;
  ...
  const filteredUsers = useMemo(() => {
    ...
    switch (activeTab) {
      case "pending":
        users = allUsers.filter((u) => u.status === "pending");
        break;
      case "approved":
        users = allUsers.filter((u) => u.status === "approved");
        break;
      case "suspended":
        users = allUsers.filter((u) => u.status === "suspended");
        break;
      default:
        users = allUsers;
    }
  ...
  return (
    <>
      ...
      <div className="pt-20 sm:pt-24 px-3 sm:px-6 lg:px-8 py-4 sm:py-8 overflow-x-hidden">
        ...
        <div
          id="tour-stats"
          className="relative mb-6 md:mb-8 overflow-hidden rounded-[1.5rem] md:rounded-[2rem] p-5 md:p-10 border shadow-2xl transition-all"
          style={{
        ...
        >
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8">
            <div className="text-center md:text-left">
              ...
              >
                Welcome back,{" "}
                <span className="bg-clip-text text- from-indigo-500 to-purple-500">
              ...
              <div className="mt-6 flex flex-wrap gap-3 justify-center md:justify-start">
                <button
                  onClick={() => navigate("/OwnersDashboard/wardens")}
                  className="px-4 md:px-5 py-2 md:py-2.5 rounded-xl bg-indigo-600 text-white font-bold text-xs md:text-sm shadow-lg shadow-indigo-500/30 hover:scale-105 transition-transform flex items-center gap-2"
                >
                  <ShieldCheck size={14} className="md:w-4 md:h-4" /> Manage
                  Wardens
                </button>
                <button
                  onClick={() => setIsAddModalOpen(true)}
                  className="px-4 md:px-5 py-2 md:py-2.5 rounded-xl border font-bold text-xs md:text-sm hover:bg-indigo-500/5 transition-all flex items-center gap-2"
                  style={{
                    borderColor: "var(--border-primary)",
              ...
              <div className="flex gap-4 md:gap-8">
                <div className="text-center">
                  <div className="p-3 md:p-5 rounded-xl md:rounded-2xl bg-indigo-500/10 border border-indigo-500/20 shadow-inner">
                    <p className="text-xl md:text-2xl font-black text-indigo-600 leading-none">
                      {pendingCount}
                    </p>
                    <p className="mt-1 md:mt-1.5 text-[10px] md:text-xs font-bold text-indigo-600 uppercase tracking-widest">
                      Pending
                    </p>
                  </div>
                </div>
        ...
        <div
          id="tour-actions"
          className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6 md:mb-8"
        >
          {[
            {
              title: "Wardens",
              ...
              cls: "bg-indigo-500/10 text-indigo-600 group-hover:bg-indigo-600",
            },
            {
              title: "Students",
              icon: Users,
              path: "/OwnersDashboard/students",
              cls: "bg-blue-500/10 text-blue-600 group-hover:bg-blue-600",
            },
            {
              title: "Analytics",
              icon: BarChart3,
              path: "/OwnersDashboard/analytics",
              cls: "bg-violet-500/10 text-violet-600 group-hover:bg-violet-600",
            },
            {
              title: "Support Tickets",
              icon: Ticket,
              path: "/OwnersDashboard/support-tickets",
              cls: "bg-orange-500/10 text-orange-600 group-hover:bg-orange-600",
            },
          ].map((action, idx) => (
            <button
              key={idx}
            ...
            >
              <div
                className={`p-3 md:p-4 rounded-xl md:rounded-2xl ${action.cls} group-hover:text-white transition-all duration-300`}
              >
                <action.icon className="w-5 h-5 md:w-6 md:h-6" />
              </div>
              <h3
                className="mt-3 md:mt-4 text-[11px] md:text-xs font-black uppercase tracking-widest"
                style={{ color: "var(--text-primary)" }}
              >
                {action.title}
              </h3>
        ...
        <section id="tour-approval-board">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pt-4">
            <div>
              ...
              >
                List of Collages manage By 'ADMIN' registrations securely
              </p>
            </div>
            <button
              type="button"
